      ...
      <div className="px-10 mt-20">
        <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-12 gap-3">
              {boxOfficeList?.danhSachGhe?.map((ghe) => (
                <Chair key={ghe.maGhe}>
                  <p
                    className={cn(
                      "Chair border border-dark rounded d-flex mt-3",
                      {
                    ...
                    onClick={() => {
                      if (booking?.includes(ghe.tenGhe)) {
                        setInfoBooking((prev) =>
                          prev.filter((e) => e.name !== ghe.tenGhe)
                        );

                        setBooking((prevState) =>
                          prevState.filter((e) => e !== ghe.tenGhe)
                        );
                        return;
                      }
                      ...
                      setInfoBooking([
                        ...infoBooking,
                        { name: ghe.tenGhe, price: ghe.giaVe },
                      ]);
          ...
          <div className="!ml-18">
            ...
            <Table
              border={1}
              className="!table-auto border-collapse border border-slate-400 text-center"
            >
              {infoBooking.length ? (
                <thead>
            ...
            {infoBooking.length ? (
              <Button
                className="!font-600 !h-[50px] !mt-20 "
                type="primary"
                onClick={() => {
                  const listBooking = boxOfficeList?.danhSachGhe?.filter((e) =>
